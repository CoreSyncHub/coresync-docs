...
const DocSidebarMobileSecondaryMenu: NavbarSecondaryMenuComponent<Props> = ({ sidebar }) => {
  ...
  const renderSidebarItem = (item: PropSidebarItem, depth = 0): React.ReactElement => {
    // LINK ITEM
    if (item.type === 'link') {
      const href = (item as any).href || '';
          ...
          <div className="custom-sidebar__collapsible">
            ...
            >
              {items.map((subItem: PropSidebarItem, idx: number) => (
                <li key={idx}>{renderSidebarItem(subItem, depth + 1)}</li>
              ))}
            </ul>
          </div>
        );
  ...
  return (
    <div
      className={clsx(ThemeClassNames.docs.docSidebarMenu, 'custom-sidebar custom-sidebar--mobile')}
    >
      <div className="custom-sidebar__content">{categories.map(renderCategory)}</div>
    </div>
  );
};
